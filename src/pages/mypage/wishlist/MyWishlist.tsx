...
function getStatusBadge(status: string) {
  switch (status) {
    ...
    case "PROCESSING":
    return { text: "진행중", chip: "bg-emerald-50 text-emerald-700 ring-emerald-100" };
    case "NOTSELLED":
    return { text: "유찰", chip: "bg-zinc-50 text-zinc-600 ring-zinc-100" };
    case "SELLED":
    return { text: "낙찰", chip: "bg-violet-50 text-violet-700 ring-violet-100" };
    default:
...
const MyWishlist = () => {
  ...
  useEffect(() => {
    loadProductsByWishlist(appliedPage, appliedSize);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, [appliedPage, appliedSize, loadProductsByWishlist]);
...
//       setLoading(false);
  ...
  const hasActiveFilter = appliedStatus !== "ALL" || !!appliedSearch || appliedSize !== 10;
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-zinc-500">
                  ...
                  <span className="ml-2 text-zinc-400">
                    (현재 페이지 {wishlist.length}개 · 표시 {visibleProducts.length}개)
                  </span>
                </p>
        ...
        <div className="mt-5">
          {loading ? (
            ...
            <div className="rounded-3xl bg-white/70 backdrop-blur ring-1 ring-black/5 shadow-sm p-12 text-center">
              ...
              <p className="mt-5 text-zinc-900 font-extrabold text-xl">찜한 경매가 없습니다</p>
              <p className="mt-2 text-sm text-zinc-500">검색어 또는 상태를 바꿔보세요.</p>
              <button
                type="button"
                onClick={resetFilters}
                      ...
                      >
                        <div className="p-4">
                          <div className="relative overflow-hidden rounded-2xl ring-1 ring-black/5">
                            ...
                            <div className="absolute left-3 right-3 top-3 z-20 flex items-center justify-between gap-3"> <div className="min-h-8 flex items-center"> <TimePill status={product.status} auctionStartTime={product.auctionStartTime} auctionEndTime={product.auctionEndTime} /> </div>
                              ...
                              return (
                                <div className="flex items-center ">
                                  <div className={cn("h-8 px-3 rounded-full text-[11px] font-extrabold flex items-center ring-1 shadow-sm", badge.chip)}>
                                    {badge.text}
                                  </div>
                                  ...
                                  <button
                                    ...
                                    className={cn(
                                      ...
                                      (!wid || isRemoving) && "opacity-40 cursor-not-allowed hover:bg-transparent active:scale-100"
                                    )}
                                  >
                                    <Heart
