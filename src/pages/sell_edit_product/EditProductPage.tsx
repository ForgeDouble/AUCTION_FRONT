...
const EditProductPage = () => {
  ...
  const [images, setImages] = useState<ProductEditImageDto[]>([]);
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImages: NewImageDto[] = files.map((file) => ({
  ...
  const loadProduct = async () => {
    try {
      ...
      const raw = unformatNumber(String(result.price));
      const onlyNumber = raw.replace(/[^0-9]/g, "");
      const formatPrice = formatNumber(onlyNumber);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 새로 추가할 파일들만 추출
      const addFiles = images
        .filter((img): img is NewImageDto => !img.isExisting)
        .map((img) => img.file);

      // 삭제할 이미지 ID 목록
      const deleteIds = deletedImageIds;

      // 현재 이미지들의 순서 (기존 이미지 ID만)
      const orderIds = images
