...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const myEmail = useMemo(() => normEmail(userEmail), [userEmail, isAuthenticated]);
    ...
    (roomId: string) => {
      ...
      const sub = clientRef.current.subscribe("/topic/chat/room/" + roomId, (frame) => {
        try {
          ...
          const senderEmail = normEmail(p.senderId ?? p.sender ?? p.senderEmail);
          ...
          const senderUserId =
            typeof p.senderUserId === "number"
              ? p.senderUserId
              : typeof p.sender?.userId === "number"
                ? p.sender.userId
                : null;
          ...
          const msg: ChatMessageDto = {
            ...
            senderUserId,
            senderNickname: String(p.senderNickname ?? p.sender?.nickname ?? ""),
            senderProfileImageUrl: String(p.senderProfileImageUrl ?? p.sender?.profileImageUrl ?? ""),
            content,
            createdAt,
            type,
            mine,
          ...
          // 방 리스트의 lastMessage/lastAt/unread까지 같이 동기화
          setRooms((prev) => {
            const next = prev.map((r) => {
              if (r.roomId !== roomId) return r;

              const isCurrent = roomId === currentRoomId;
              const nextUnread = isCurrent ? 0 : (r.unread || 0) + 1;

              return {
                ...r,
                lastMessage: type === "IMAGE" ? "이미지를 보냈습니다." : (content || r.lastMessage),
                lastAt: createdAt,
                unread: nextUnread,
              };
            });

            // 최근 메시지 온 방을 위로 올리고 싶으면 정렬
            next.sort((a, b) => {
  ...
  useEffect(() => {
    ...
    const wsUrl = (import.meta.env.VITE_API_BASE || "").replace(/\/+$/, "") + "/ws";
      ...
      list.map(async (r: any) => {
        try {
          ...
          const others = (Array.isArray(members) ? members : []).filter(
            (m) => normEmail(m?.email) && normEmail(m.email) !== me,
          );
  ...
  }, [connected, currentRoomId, subscribeRoom]);
  ...
  const selectRoom = useCallback(
    async (roomId: string) => {
      ...
      if (connected) subscribeRoom(roomId);
      ...
      try {
        ...
        const corrected = list.map((m) => ({
          ...m,
          mine: normEmail(m.senderId) === myEmail,
        }));
    ...
    [token, myEmail, connected, subscribeRoom],
  );
  ...
  // 방에서 나갈 때 presence exit
  useEffect(() => {
    if (!token) return;
    if (!connected || !currentRoomId) return;

    return () => {
      exitRoom(token).catch(() => {});
    };
