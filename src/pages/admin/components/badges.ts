...
import type { BackendReportStatus, ReportCategory } from "../adminTypes";

// 신고 관련 부분 컴포넌트 + 옥션(경매) 컴포넌트

export function reportStatusBadge(st: BackendReportStatus): {
  label: string;
  cls: string;
} {
...
export function categoryBadge(cat: ReportCategory): {
  label: string;
  cls: string;
} {
...
export function pendingRiskBadge(pendingCount: number): {
  label: string;
  cls: string;
} {
  if (pendingCount >= 5)
    return { label: "임계치", cls: "bg-red-50 text-red-700 border-red-200" };
  if (pendingCount >= 3)
    return {
      label: "주의",
      cls: "bg-orange-50 text-orange-700 border-orange-200",
  ...
  if (pendingCount >= 1)
    return { label: "관찰", cls: "bg-blue-50 text-blue-700 border-blue-200" };
...
export function auctionBadge(status?: string) {
  ...
  return (
    map[s] ?? { label: s, cls: "bg-gray-50 text-gray-700 border-gray-200" }
  );
}
