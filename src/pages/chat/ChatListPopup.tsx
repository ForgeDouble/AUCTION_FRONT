  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#fbf3ff] to-[#f7eefb]">
      <div className="sticky top-0 z-10 bg-white/70 backdrop-blur border-b border-black/5 px-4 py-3 flex items-center justify-between">
        ...
        <button
          onClick={openAdminAndSelect}
          className="flex items-center gap-1 text-sm bg-purple-600 text-white px-3 py-1.5 rounded-full hover:bg-purple-700"
          title="관리자에게 문의하기"
        >
          <Plus size={16} /> 문의하기
        </button>
      ...
      <div className="p-2">
        <div className="bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden">
          {rooms.length === 0 && (
            <div className="p-8 text-center text-gray-500">채팅 내역이 없습니다.</div>
          )}
          {rooms.map((r) => (
            <button
              key={r.roomId}
